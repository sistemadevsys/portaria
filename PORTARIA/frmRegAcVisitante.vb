...
Public Class frmRegAcVisitante
    ...
    Dim opc As Integer
    Dim chave As String
    Dim FAS011, EMP002 As New TextBox
    Dim ARM002, VEI002, FUN035, FUN003, IMG006 As New TextBox
...
#Region "FUNÇÕES"
    ...
    Private Sub gravar(arg)
        Select Case arg
            Case 0
                ...
                Dim cmd As New MySqlCommand("INSERT INTO prt_fass (FUN002,EMP002,VEI002,FAS002,FAS003,FAS004,FAS005,FAS011,OBS)values('" & FUN002.Text & "','" & EMP002.Text & "','" & btnCarro.Text & "','" & Format(data_hoje, "yyyy-MM-dd") & "','" & DateTime.Now.ToLongTimeString().Substring(0, 5) & "','" & DateTime.Now.ToLongTimeString().Substring(0, 5) & "','" & iUsername & "','" & FAS011.Text & "','" & FUN035.Text & "')", conexao)
                conexao.Open()
                cmd.ExecuteNonQuery()
                conexao.Close()
        End Select
    ...
    Private Sub limpaCampos()
        ...
        btnObs.Text = Nothing
        img.ImageLocation = (My.Application.Info.DirectoryPath & "\file\visitor\REGISTRO.dat")
    ...
    Private Sub filtro()
        Select Case filtroMov
            Case 0
                sqlm = "SELECT FUN002,FUN003,FUN024,FUN028 FROM prt_cadfun where FUN023 = '0' and FUN028 = 'VIS' ORDER BY FUN003,EMP002"
            Case 1
                sqlm = "SELECT FUN002,FUN003,FUN024,FUN028 FROM prt_cadfun where FUN003 like '" & chave & "%' and FUN023 = '0' AND FUN028 = 'VIS' ORDER BY FUN003"
        End Select
    End Sub
    ...
    Private Sub buscadados()
        If conexao.State = 0 Then
            conexao.Open()
        End If
        Dim comando As New MySqlCommand("select a.EMP002,a.FUN028,a.FUN004,a.FUN008,a.FUN009,a.FUN029,a.FUN030,a.FUN031,a.FUN032,a.FUN033,a.FUN034,b.EMP003 FROM prt_cadfun as a left JOIN prt_empresa as b on (a.emp002 = b.emp002) where a.FUN002 = '" & lvMovimento.SelectedItems(0).SubItems(0).Text & "'", conexao)
        Dim leitor As MySqlDataReader = comando.ExecuteReader()
        While leitor.Read()
            FUN004.Text = IIf(Convert.IsDBNull(leitor("FUN004")) = True, "", leitor("FUN004"))
            FUN008.Text = IIf(Convert.IsDBNull(leitor("FUN008")) = True, "", leitor("FUN008"))
    ...
    Private Sub lvMovimento_SelectedIndexChanged(sender As Object, e As EventArgs) Handles lvMovimento.SelectedIndexChanged
        Try
            If lvMovimento.SelectedItems.Count > 0 Then
                ...
                img.ImageLocation = (My.Application.Info.DirectoryPath & "\file\visitor\" & Me.lvMovimento.SelectedItems(0).SubItems(2).Text & ".dat")
...
#Region "BOTÕES"
    ...
    Private Sub btnEdita_Click(sender As Object, e As EventArgs) Handles btnEdita.Click
        alterado = 0
        frmVisitante.FUN002.Text = FUN002.Text
        frmVisitante.opcao.Text = "EDITAR"
        frmVisitante.ShowDialog()
        iFUN028 = "VIS"
        filtroMov = 0
        If alterado > 0 Then
