...
Public Class frmLocatario
    ...
    Dim CADP001, CADP023, CADP026, REGL006 As New TextBox
    ...
    Private Sub carregalist(arg)
        Select Case arg
            ...
            Case 1
                ...
                conexao.Open()
                Dim cmd As MySqlCommand = New MySqlCommand(" select REGL001,CADP002, CADP003, REGL003, REGL004, REGL006, REGL007 FROM prt_regloc where CADL002 = '" & CADP002.Text & "'", conexao)
                Dim leitor As MySqlDataReader = cmd.ExecuteReader()
                ...
                Do While leitor.Read()
                    ...
                    item.SubItems.Add(leitor("CADP002").ToString)               ' 1 - REGISTRO NO SISTEMA
                    item.SubItems.Add(leitor("CADP003"))                        ' 2 - CÓDIGO NA TABELA PRT_CADPRO
                    item.SubItems.Add(leitor("REGL003")) ' 3 - DATA DE INICIO
                    item.SubItems.Add(leitor("REGL004")) ' 4 - DATA PREVISTA PARA FINALIZAR
                    item.SubItems.Add(leitor("REGL006")) ' 5 - STATUS 
                    item.SubItems.Add(IIf(Convert.IsDBNull(leitor("REGL007")) = True, "", leitor("REGL007"))) ' DATA DO ENCERRAMENTO
                    popReg += 1
                Loop
                conexao.Close() ' fecha a conexação com o banco de dados
                leitor.Dispose()
        End Select
    End Sub
    Private Sub lvReloc_Click(sender As Object, e As EventArgs) Handles lvReloc.Click
        Call CORLIST(0)
    End Sub
    Private Sub lvReloc_SelectedIndexChanged(sender As Object, e As EventArgs) Handles lvReloc.SelectedIndexChanged
        Try
            If lvReloc.SelectedItems.Count > 0 Then
                Me.DTini.Text = Me.lvReloc.SelectedItems(0).SubItems(3).Text
                Me.DTfim.Text = Me.lvReloc.SelectedItems(0).SubItems(4).Text
                Me.REGL006.Text = Me.lvReloc.SelectedItems(0).SubItems(5).Text
                Me.DTenc.Text = Me.lvReloc.SelectedItems(0).SubItems(6).Text
                If REGL006.Text = "1" Then
                    status.Text = "EM ANDAMENTO"
                    Label7.Visible = False
                    DTenc.Visible = False
                Else
                    status.Text = "E N C E R R A D O"
                    Label7.Visible = True
                    DTenc.Visible = True
                End If
            End If
    ...
    Private Sub tratatab()
        Select Case Tabelas.SelectedTab.Name
            ...
            Case "Tab1"
                ...
                Repositorio.origem.Text = "LOCATARIOS"

                abriform(Of frmAutorizados)() ' ABRE O FORMULÁRIO DENTRO DO PAINEL
            Case "Tab2"
                Panel3.Visible = True
                tarja.BackColor = Color.FromArgb(0, 21, 68)
                tarja.Text = "Relação de Pessoas que utilizaram o apartamento"

                Repositorio.iCADP002.Text = CADP002.Text
                Repositorio.iORIGEM.Text = "LOC" ' filtro cadp037
        ...
        Select Case arg
            Case 0
                ...
                If conexao.State = 0 Then
                    conexao.Open()
                End If
                sqlmaior = "Select Max(CADP002) From prt_cadpro where CADP026 = 'LOC' AND CADP037 = '3'"
    ...
    Private Sub filtrar()
        Select Case filtro
            Case 99 ' TODOS OS REGISTROS
                sql = "SELECT CADP001,CADP002,CADP003,CADP004 FROM PRT_CADPRO WHERE CADP028 = '0' AND CADP026 = 'LOC' AND CADP037 = '3' ORDER BY CADP002"
            Case 0 ' FILTRA POR CÓDIGO DA UNIDADE
                sql = "SELECT CADP001,CADP002,CADP003,CADP004 FROM PRT_CADPRO WHERE CADP002 like '" & chave & "%' AND CADP028 = '0' AND CADP026 = 'LOC' AND CADP037 = '3' order by CADP002"
            Case 1 ' FILTRA PELO NOME DO PROPRIETÁRIO
                sql = "SELECT CADP001,CADP002,CADP003,CADP004 FROM PRT_CADPRO WHERE CADP003 like '" & chave & "%' AND CADP028 = '0' AND CADP026 = 'LOC' AND CADP037 = '3' order by CADP003"
        End Select
    End Sub
    ...
    Private Sub CADP017_KeyDown(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyEventArgs) Handles CADP017.KeyDown
        ...
        If KeyCode = System.Windows.Forms.Keys.Return Then
            If CADP017.TextLength = 0 Then
                MetroFramework.MetroMessageBox.Show(Me, " Informe pelo menos uma conta de email para envio de comunicados ao locatário! ", "" & legenda & "", MessageBoxButtons.OK, MessageBoxIcon.Error)
                CADP017.Focus()
            Else
                CADP020.Focus()
            End If
            ...
            If grava = "Sim" Then
                If (MetroFramework.MetroMessageBox.Show(Me, " Deseja mesmo gravar as informações ? ", "" & legenda & "", MessageBoxButtons.YesNo, MessageBoxIcon.Information)) = MsgBoxResult.Yes Then
                    Try
                        ...
                        Dim sql As String = "INSERT INTO PRT_CADPRO(CADP002,CADP003,CADP004,CADP005,CADP006,CADP007,CADP008,CADP009,CADP010,CADP011,CADP024,CADP013,CADP014,CADP015,CADP016,CADP017,CADP018,CADP020,CADP026,CADP027,CADP037)VALUES('" & CADP002.Text & "','" & CADP003.Text & "','" & CADP004.Text & "','" & CADP005.Text & "','" & CADP006.Text & "','" & CADP007.Text & "','" & CADP008.Text & "','" & CADP009.Text & "','" & CADP010.Text & "','" & CADP011.Text & "','" & CADP024.Text & "','" & CADP013.Text & "','" & CADP014.Text & "','" & CADP015.Text & "','" & CADP016.Text & "','" & CADP017.Text & "','" & Format(CADP018.Value, "yyyy-MM-dd") & "','" & CADP020.Text & "','LOC','LOCATARIO','3')"
