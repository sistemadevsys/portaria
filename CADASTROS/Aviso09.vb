...
#Region "FUNÇÕES"
    Private Sub carregaList()
        ...
        conexao.Open()
        Dim cmd As MySqlCommand = New MySqlCommand("select IMG001,IMG003,IMG004,USU002,IMG005 from prt_imagem where IMG002 = '" & FUN002.Text & "' ORDER BY IMG003 DESC", conexao)
        ...
        If pop = 0 Then
            If IMG006.Text = "VIS" Then
                imgbanco.ImageLocation = (My.Application.Info.DirectoryPath & "\file\visitor\REGISTRO.dat")
            ElseIf IMG006.Text = "FUN" Or IMG006.Text = "PRE" Or IMG006.Text = "EMP" Then
                imgbanco.ImageLocation = (My.Application.Info.DirectoryPath & "\file\employee\REGISTRO.dat")
            End If
        End If
    ...
    Public Sub tempValue()
        ...
        If IMG006.Text = "VIS" Then
            img.Save(Application.StartupPath + "\file\visitor\" & FileName & ".dat", objImageCodecInfo(1), objEncParams)
        ElseIf IMG006.Text = "FUN" Or IMG006.text = "PRE" Or IMG006.text = "EMP" Then
            img.Save(Application.StartupPath + "\file\employee\" & FileName & ".dat", objImageCodecInfo(1), objEncParams)
        End If
    ...
    Private Sub gravar(arg)
        Select Case arg
            Case 0
                ...
                Dim cmd As New MySqlCommand("INSERT INTO prt_imagem (IMG002,IMG003,IMG004,IMG006,IMG005,USU002)VALUES('" & FUN002.Text & "','" & Format(data_hoje, "yyyy-MM-dd") & "','" & DateTime.Now.ToLongTimeString & "','" & IMG006.Text & "','" & FileName & "','" & iUsername & "')", conexao)
                conexao.Open()
                cmd.ExecuteNonQuery()
                conexao.Close()
            Case 1
                ...
                If conexao.State = ConnectionState.Open Then conexao.Close()
                Dim cmd As New MySqlCommand("update prt_cadfun set FUN024 = '" & lvFotos.SelectedItems(0).SubItems(4).Text & "' WHERE FUN002 = '" & FUN002.Text & "'", conexao)
                conexao.Open()
                cmd.ExecuteNonQuery()
                conexao.Close()
        End Select
    ...
    Private Sub lvFotos_SelectedIndexChanged(sender As Object, e As EventArgs) Handles lvFotos.SelectedIndexChanged
        Try
            If lvFotos.SelectedItems.Count > 0 Then
                ...
                FUN024.Text = Me.lvFotos.SelectedItems(0).SubItems(4).Text

                If IMG006.Text = "VIS" Then
                    imgbanco.ImageLocation = (My.Application.Info.DirectoryPath & "\file\visitor\" & FUN024.Text & ".dat")
                ElseIf IMG006.Text = "FUN" Or IMG006.text = "PRE" Or IMG006.text = "EMP" Then
                    imgbanco.ImageLocation = (My.Application.Info.DirectoryPath & "\file\employee\" & FUN024.Text & ".dat")
                End If
